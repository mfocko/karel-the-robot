...
START_TEST(when_facing_north_then_return_true)
    {
        _karel.direction = NORTH;
        _karel.is_running = true;

        ck_assert_int_eq(not_facing_south(), true);
    }
END_TEST
...
START_TEST(when_facing_south_then_return_false)
    {
        ...
        ck_assert_int_eq(not_facing_south(), false);
    }
END_TEST
...
Suite *suite_not_facing_south() {
    Suite *suite = suite_create("Suite not_facing_south");

    TCase *tc = tcase_create("Basic test");
    tcase_add_test(tc, when_facing_north_then_return_true);
